...
To push this entire project as a clean, single repository on GitHub:
...
2. **Stage files and commit**:
   ```bash
   git add .
   git commit -m "Initial commit: VisionHire AI Resume Parser and Scorer Monorepo"
   ```

3. **Link remote origin & push**:
   ```bash
   git remote add origin https://github.com/YOUR_USERNAME/visionhire-ai.git
   git branch -M main
